...
import { data } from "../../src/home/json.js";
...
const Home = () => {
  ...
  function add_shop(product) {
    let updatedCart = [...cartItems];

    const exists = updatedCart.some((item) => item.id === product.id);
    if (!exists) {
      updatedCart.push(product);
      localStorage.setItem("cartItems", JSON.stringify(updatedCart)); // ✅ To‘g‘ri nom
      ...
      <header className="flex flex-col items-center justify-center w-[100%] gap-[20px]">
        <div className="flex items-center justify-center w-[100%] gap-[57px]">
          ...
          <button className="flex items-center justify-center bg-[#FFBE1F] rounded-[12px] h-[44px] pl-[10px] pr-[10px] gap-[5px]">
            ...
            <span className="hidden lg:block text-black text-[16px] font-[600]">
        ...
        <div className="w-[100%]">
          <div className="flex items-center justify-center w-[100%] whitespace-nowrap gap-[12px]">
            <div className="cursor-box move text-white">
            ...
            <div className="cursor-box move text-white">Audio uskunalar</div>
            <div className="cursor-box move text-white">Transport</div>
            <div className="cursor-box move text-white">Setlar</div>
            <div className="cursor-box move text-white">Maishiy texnika</div>
            <div className="cursor-box move text-white">Oshxona jihozlari</div>
          </div>
        </div>
      </header>
        ...
        <div className="flex flex-wrap items-center justify-start gap-[20px] w-[100%]">
          {data.map((product) => (
            <div className="flex flex-col items-start justify-between w-[250px] h-[450px] cursor-pointer gap-[5px]">
              ...
              <p className="text-[15px] font-[400]">{product.info}</p>
              <div className="rounded-[10px] pl-[10px] pr-[10px] bg-[#FFEDC2] border-[#FFCA48] border-[1px]">
                <p className="">dan {product.oyiga} so'm/oy</p>
              </div>
              <del className="text-[15px] text-[#A5B1BB]">
                {product.old_price} so'm
              </del>
              <p className="text-[#FF4444] text-[18px]">
                {product.active_price} so'm
              </p>
              <button
                className="flex items-center justify-center bg-[#FFD671] h-[50px] rounded-[10px] pl-[10px] pr-[10px] gap-[10px] w-[100%]"
                onClick={() => add_shop(product)}
              >
                <svg
                  width="24"
                  height="24"
